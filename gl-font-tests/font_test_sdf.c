...
#define BPP 1
    ...
    unsigned char* data = malloc(f->face->glyph->bitmap.width * f->face->glyph->bitmap.rows * BPP);
    int width  = f->face->glyph->bitmap.width,
        height = f->face->glyph->bitmap.rows,
        bpp    = BPP;
    
    sdfBuildDistanceField(data, width, 10.f,
                          f->face->glyph->bitmap.buffer, width, height, width);
...
int main(int argc, const char* argv[]) {
  ...
  font_t test;
  init_font(&test, "/Library/Fonts/Times New Roman.ttf", 72);
  LOAD_DEF_CHAR_RANGE(&test);
  
  string_t str;
  init_string(&str, "SEE YOU SPACE COWBOY...", &test);
  
  GLuint shader = load_shader_file("font_test_sdf.vert.glsl", "font_test_sdf.frag.glsl");
  ...
  while (running) {
    ...
    if (x_pos < -3000.f)
      x_pos = 10.f;
    glUniform2f(glGetUniformLocation(shader, "position"), x_pos, 10.f);
